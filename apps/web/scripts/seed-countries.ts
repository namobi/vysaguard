/**
 * Seed all ~195 countries into the countries table.
 *
 * Only sets name, slug, iso2, iso3.
 * Theme fields are left NULL for non-destination countries.
 * Existing countries (matched by iso2) are NOT overwritten.
 *
 * Run with: node --env-file=.env.local --import tsx scripts/seed-countries.ts
 */
...
// Full list of UN-recognized countries + commonly referenced territories
...
async function seedCountries() {
  try {
    ...
    // Fetch existing countries to skip duplicates
    const { data: existing } = await supabase
      .from("countries")
      .select("iso2");

    const existingIso2 = new Set((existing ?? []).map((c: any) => c.iso2));

    const toInsert = COUNTRIES.filter((c) => !existingIso2.has(c.iso2)).map(
      (c) => ({
        name: c.name,
        slug: slugify(c.name),
        iso2: c.iso2,
    ...
    const skipped = COUNTRIES.length - toInsert.length;
    console.log(`  Skipping ${skipped} countries (already exist)`);
